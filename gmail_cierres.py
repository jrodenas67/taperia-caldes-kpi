...
_RE_EFECTIVO  = re.compile(r"Ventas\s+efectivo\s+([0-9.,]+)\s*€", re.IGNORECASE)
_RE_TARJETA   = re.compile(r"Ventas\s+tarjeta\s+([0-9.,]+)\s*€", re.IGNORECASE)
_RE_TICKETR   = re.compile(r"Ventas\s+Ticket\s+Restaurant\s+([0-9.,]+)\s*€", re.IGNORECASE)
...
def _parse_pdf(raw: bytes) -> dict | None:
    """Extrae numero_cierre, fecha_inicio y desglose de ventas del PDF."""
    try:
    ...
    m_efe = _RE_EFECTIVO.search(text)
    m_tar = _RE_TARJETA.search(text)
    m_tkt = _RE_TICKETR.search(text)
    ...
    return {
        ...
        "fecha":    fecha,
        "total":    total,
        "efectivo": _to_float(m_efe.group(1)) if m_efe else 0.0,
        "tarjeta":  _to_float(m_tar.group(1)) if m_tar else 0.0,
        "tickets":  _to_float(m_tkt.group(1)) if m_tkt else 0.0,
    }
